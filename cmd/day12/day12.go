...
func countArrangements(sequence []spring, groupSizes []int, depth int, memo map[string]int) int {
	...
	case Empty:
		...
		result := countArrangements(sequence[1:], groupSizes, depth+1, memo)
		//log.Printf("%v return %v", strings.Repeat(" ", depth), result)
		return result
	case Present:
		result := countArrangementsIfPresent(sequence, groupSizes, depth, memo)
		//log.Printf("%v return %v", strings.Repeat(" ", depth), result)
		return result
	case Unknown:
		key := ""
...
func countArrangementsIfPresent(sequence []spring, groupSizes []int, depth int, memo map[string]int) int {
	if len(groupSizes) == 0 {
	...
	return countArrangements(sequence[groupSize+1:], groupSizes[1:], depth+1, memo)
}
